...
- garth 0.5.x 대응: 클래스가 아니라 모듈 함수(load_token/refresh_token/login/save_token) 사용
- garminconnect 라이브러리 유무/버전에 따라 여러 업로드 경로 시도
"""
...
import garth  # ← 모듈 방식 API

try:
    from garminconnect import Garmin  # type: ignore
except Exception:
    Garmin = None  # 없는 환경을 대비
...
def ensure_login():
    """garth 모듈 전역 클라이언트에 로그인/토큰 세팅"""
    email = os.environ.get("GARMIN_EMAIL")
    password = os.environ.get("GARMIN_PASSWORD")
    if not email or not password:
        die("GARMIN_EMAIL / GARMIN_PASSWORD 환경변수가 필요합니다.")

    # 토큰 재사용 → 실패 시 로그인 후 저장
    try:
        garth.load_token()         # ~/.garth/token.json 기본 경로
        garth.refresh_token()
        log("[INFO] 토큰 재사용 로그인 성공")
    except Exception:
        log("[INFO] 신규 로그인 시도")
        garth.login(email=email, password=password)
        garth.save_token()         # ~/.garth/token.json 에 저장
        log("[INFO] 로그인 & 토큰 저장 완료")

    return garth  # 모듈 자체를 반환(내부에 client 보유)
...
def try_upload_with_garminconnect(garth_mod, rec: WeightRecord) -> bool:
    """garminconnect 라이브러리 있으면 가능한 메서드들을 차례로 시도"""
    if Garmin is None:
        return False

    api = None

    # 새 버전: garminconnect가 garth client를 받는 경우
    gc_kwargs_candidates = []
    client = getattr(garth_mod, "client", None)
    if client is not None:
        gc_kwargs_candidates.append({"client": client})
        gc_kwargs_candidates.append({"garth": client})
    # 혹시 모듈 자체를 받도록 구현된 경우
    gc_kwargs_candidates.append({"garth": garth_mod})
    # 마지막으로 구버전(이메일/패스워드 직접 로그인)
    gc_kwargs_candidates.append({"email": os.environ.get("GARMIN_EMAIL"), "password": os.environ.get("GARMIN_PASSWORD")})

    # 인자 조합을 순서대로 시도
    for kwargs in gc_kwargs_candidates:
        try:
            if "email" in kwargs and "password" in kwargs and kwargs["email"] and kwargs["password"]:
                api = Garmin(kwargs["email"], kwargs["password"])  # type: ignore
            ...
            log(f"[WARN] garminconnect 인스턴스 생성/로그인 실패: {e}")
...
def try_upload_with_garth_http(garth_mod, rec: WeightRecord) -> bool:
    """garth의 세션으로 Garmin Connect HTTP 엔드포인트 직접 호출"""
    # POST 신규 엔트리
    try:
        payload = {
            "value": rec.weight_kg,
...
def run() -> None:
    ...
    uploaded = try_upload_with_garminconnect(garth_mod, rec)
    if not uploaded:
        log("[WARN] garminconnect 실패 → garth HTTP 시도")
        uploaded = try_upload_with_garth_http(garth_mod, rec)
